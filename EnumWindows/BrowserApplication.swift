...
extension BrowserEntity {
  func performSelectorByName<T>(name: String, defaultValue: T) -> T {
    ...
    guard rawItem.responds(to: sel) else {
      return defaultValue
    }

    let selectorResult = rawItem.perform(sel)
...
class BrowserTab: BrowserNamedEntity, Searchable, ProcessNameProtocol {
  ...
  var tabTitle: String {
    ...
    if rawItem.responds(to: #selector(NSImage.name)) {
...
class BrowserWindow: BrowserNamedEntity {
  ...
  var tabs: [BrowserTab] {
    ...
    return result.enumerated().map { index, element in
      BrowserTab(raw: element, index: index, windowIndex: self.windowIndex, processName: self.processName, bundleId: self.bundleId, fullPath: self.fullPath)
    }
  }
...
class BrowserApplication: BrowserEntity {
  ...
  var windows: [BrowserWindow] {
    ...
    return result.enumerated().map { index, element in
      BrowserWindow(raw: element, index: index, processName: self.processName, bundleId: self.bundleId, fullPath: self.fullPath)
    }
  }
}
